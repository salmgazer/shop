				...
				<React.Fragment>
					...
					>
						<div style={{ width: '80%', margin: '0 auto'}}>
							<h3 style={{fontSize: '40px', fontWeight: '400', color: '#09d3ac'}}>Create new Company</h3>
							...
							<Textarea
								...
								onChange={(e) => setState({newCompanyDescription: e.target.value})}
							...
							<Grid container spacing={1}>
								<Grid xs={5} item>
									<TextInput
										...
										onChange={(e) => setState({newCompanyLocationGPS: e.target.value})}
								...
								<Grid xs={5} item>
									<TextInput
										...
										onChange={(e) => setState({newCompanyLocation: e.target.value})}
							...
							<Combobox
								...
								onChange={selected => setState({newCompanyCategory: selected})}
							...
							>
								<Button>{state.newCompanyUserId ? users.find(u => u.id === state.newCompanyUserId).name : 'Select owner of company...'}</Button>
							</SelectMenu>
							...
							<div style={{ margin: '0 auto', marginTop: '20px'}}>
								...
								<Button onClick={async () => {
									...
									setState({ isShown: false, newCompanyName: '', newCompanyDescription: '' })
...
const EditComponent = (props) => {
	...
	return (
		<Component initialState={{ isShown: false, newCompanyName: row.name || '', newCompanyDescription: row.description || '' }}>
			{({ state, setState }) => (
				<React.Fragment>
					<SideSheet
						isShown={state.isShown}
					...
					>
						<div style={{ width: '80%', margin: '0 auto'}}>
							...
							<div style={{ margin: '0 auto', marginTop: '20px'}}>
								<Button onClick={() => setState({ isShown: false })} intent='danger'>Cancel</Button>
								<Button onClick={() => {
									updateRecord({ id: row[keyFieldName], name: state.newCompanyName, description: state.newCompanyDescription });
...
const Companies = (props) => {
	const {companies, allCompanies, database, company, users, history, search, DrawerIcon, modelName} = props;
	...
	const createRecord = async (companyToCreate) => {
		await database.action(async () => {
			...
			console.log(`Created ${newCompany.name}, owned by ${companyToCreate.userId}`);
		});
	};
			...
			<div id="main-area-company">
				...
				<div id="side-nav">
					...
					<div id="nav-list">
						...
						<button className="nav-item" onClick={() => history.push('users')}>Users</button>
					</div>
				</div>
				<div id="main-body">
					<div>
...
const EnhancedCompanies = withDatabase(withObservables(["searchConfig"], ({ database, searchConfig }) => ({
	companies: database.collections.get(Company.table).query(Q.where(searchConfig.key, Q.like(`%${Q.sanitizeLikeString(searchConfig.value)}%`))).observe(),
	allCompanies: database.collections.get(Company.table).query().observe(),
...
export default class Parent extends React.Component {
	...
	render() {
		...
		return (
			<EnhancedCompanies searchConfig={this.state} modelName={modelName} company={company} DrawerIcon={DrawerIcon} search={this.search} />
		);
	}
}
