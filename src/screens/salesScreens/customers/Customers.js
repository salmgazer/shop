...
const CreateComponent = (props) => {
	...
	return (
		<Component initialState={{ isShown: false, newCustomerName: '', newCustomerNotes: '', newCustomerPhone: '' }}>
			{({ state, setState }) => (
				<React.Fragment>
					<SideSheet
					...
					>
						<div style={{ width: '80%', margin: '0 auto'}}>
							<h3 style={{fontSize: '40px', fontWeight: '400', color: '#09d3ac'}}>Create new Customer</h3>
							<label>Name: &nbsp;&nbsp;&nbsp;</label>
							<TextInput
								required
								name="name"
							...
							<TextInput
								...
								style={{marginBottom: '20px'}}
							/>
							<br/><label>Note about customer:</label><br/>
							<Textarea
								name="note"
							...
							<div style={{ margin: '0 auto', marginTop: '50px'}}>
								...
								<Button onClick={async () => {
									await createRecord({ name: state.newCustomerName, note: state.newCustomerNotes, phone: state.newCustomerPhone });
									setState({ isShown: false, newCustomerName: '', newCustomerNotes: '', newCustomerPhone: '' })
...
const EditComponent = (props) => {
	...
	return (
		<Component initialState={{ isShown: false, newCustomerName: row.name || '', newCustomerNotes: row.note || '', newCustomerPhone: row.phone || '' }}>
			{({ state, setState }) => (
				<React.Fragment>
					<SideSheet
						isShown={state.isShown}
					...
					>
						<div style={{ width: '80%', margin: '0 auto'}}>
							...
							<div style={{ margin: '0 auto', marginTop: '20px'}}>
								<Button onClick={() => setState({ isShown: false })} intent='danger'>Cancel</Button>
								<Button onClick={() => {
									updateRecord({ id: row[keyFieldName], name: state.newCustomerName, note: state.newCustomerNotes, phone: state.newCustomerPhone });
...
const Customers = (props) => {
	const {user, company, customers, users, database, history, parentLocation, search, DrawerIcon, modelName} = props;
			...
			<div id="main-area">
				...
				<div id="side-nav">
					...
					<div id="nav-list">
						...
						<button className="nav-item active" onClick={() => history.push('customers')}>Customers</button>
					</div>
...
const EnhancedCustomers = withDatabase(withObservables(["searchConfig"], ({ database, searchConfig }) => ({
	customers: database.collections.get(Customer.table).query(Q.where(searchConfig.key, Q.like(`%${Q.sanitizeLikeString(searchConfig.value)}%`))).observe(),
...
class Parent extends React.Component {
	...
	render() {
		...
		return (
			<EnhancedCustomers searchConfig={this.state} modelName={modelName} company={company} user={user} DrawerIcon={DrawerIcon} search={this.search} />
		);
	}
}
