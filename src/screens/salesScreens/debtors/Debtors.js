...
const CardListItem = props => {
  ...
  sales
    ...
    .forEach(sale => (totalArrears += sale.arrears));
...
const EnhancedCardListItem = withDatabase(
  withObservables([], ({ database, sale }) => ({
    customer: database.collections
      .get(Customer.table)
      .findAndObserve(sale.customerId)
  }))(CardListItem)
);
...
class Debtors extends React.Component {
  ...
  render() {
    ...
    const filteredCustomers = customers.filter(c =>
      uniqueSalesWithArrears.map(s => s.customerId).includes(c.id)
    );

    if (selectedCustomerId && selectedCustomerId !== "all") {
      filteredSales = uniqueSalesWithArrears.filter(
...
const EnhancedDebtors = withDatabase(
  withObservables([], ({ database, company }) => ({
    sales: database.collections
      .get(Sale.table)
      .query(
        Q.where("payment_status", Q.notIn(["full payment"])),
        Q.where("type", "sale"),
