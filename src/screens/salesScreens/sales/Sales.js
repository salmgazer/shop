          ...
          <table>
            ...
            <tbody>
              {saleEntries.map(saleEntry => {
				  ...
				  product = products.find(p => p.id === saleEntry.product.id);
				} else {
...
const SaleEntryComponentRaw = props => {
  ...
  return (
    <Grid container spacing={1} className="sale-entry" style={{borderColor: 'lightblue'}}>
      <Grid item style={{ marginRight: "5px", width: '240px', fontSize: '12px' }}>
		<ReactSelect
		...
		>
			<Row>
				...
				<Col span={14} id="actual-sales-view">
					...
					<div style={{overflow: 'scroll', height: '500px', marginBottom: '20px', overflowX: 'hidden'}}>
...
const Sales = props => {
  ...
  const productsCollection = database.collections.get(Product.table);
	...
	/*
	const myCollection = database.collections.get(Product.table);
  ...
  const createRecord = async (options) => {
  	const { salesEntries, selectedCustomer, discount, saleType } = options;
      	...
      	if (newSale && newSale.id) {
        	for (const saleEntry of salesEntries) {
				...
				for (const pp of currentProductPrices
