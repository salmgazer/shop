...
const CardListItem = (props) => {
	const {entry, company, model, displayName, createdBy, EditComponent, deleteRecord, updateRecord, keyFieldName, modelName, displayNameField, fieldNames, database} = props;
	...
	return (
		<Grid container spacing={1}>
						...
						<React.Fragment>
							...
							>
								<div style={{ width: '80%', margin: '0 auto'}}>
									<h3 style={{fontSize: '40px', fontWeight: '400', color: '#09d3ac'}}>Details of {displayName || modelName}</h3>
									{
										fieldNames.map(field => {
												...
												return <h5 style={{fontSize: '20px', fontWeight: 'lighter'}} key={field.name}>
													<b style={{fontWeight: '400'}}>{capitalize(field.label)}</b>
													: {<Chip label={value} variant="outlined" /> || ''}</h5>
											}
										)
									}
			...
			<Grid item xs={3} style={{ marginTop: '16px'}}>
				<div style={{color: '#7B8B9A', fontSize: '14px'}}>{createdBy ? createdBy.name : ''}</div>
			</Grid>
...
class CardList extends React.Component {
	...
	render() {
		const {entries, EditComponent, deleteRecord, updateRecord, fieldNames, database, displayName,
			displayNameField, modelName, columns, tableData, model, keyFieldName, search, company, user, users } = this.props;
			...
			<div>
				<Grid container spacing={1} style={{marginBottom: '15px', width: '85%', marginRight: '50px', float: 'right'}}>
					...
					<Grid item xs={4} style={{color: 'darkgrey', marginBottom: '-20px', marginLeft: '10px', marginRight: '-18px'}}>
						<SearchInput
							width='90%'
							placeholder={`Search ${displayName} by name`}
				...
				<div className="list-div">
					<Grid container spacing={1} id="list-area" className="table-shadow">
						<Table
							columns={columns}
