...
const CardListItem = (props) => {
	const {entry, company, user, category, brand, brands, categories, EditComponent, deleteRecord, updateRecord, keyFieldName, modelName, displayNameField, fieldNames, database} = props;
								...
								<div style={{ width: '80%', margin: '0 auto'}}>
									<h3 style={{fontSize: '40px', fontWeight: '400', color: '#09d3ac'}}>Details of {modelName}</h3>
									{
										fieldNames.map(field => {
												...
												if (field.name === 'brand') {
													console.log("WE GOT IN HERE TOO =>>>>>");
													value = brand.name;
...
class CardList extends React.Component {
	...
	render() {
		const {entries, EditComponent, deleteRecord, updateRecord, fieldNames, database, brands, categories,
			displayNameField, modelName, keyFieldName, search, company, user, users } = this.props;
			...
			<div>
				<Grid container spacing={1} style={{marginBottom: '15px', width: '85%', marginRight: '50px', float: 'right'}}>
					...
					<Grid item xs={2}>
						...
						{/*
							<MuiPickersUtilsProvider utils={DateFnsUtils} style={{marginTop: '-20px !important'}}>
								<KeyboardDatePicker
									margin="normal"
